...
async function render_noise(canvas, color, density, alpha, complexity) {
  let base_frag_file = await fetch('noise2D.glsl');
  let base_frag = await base_frag_file.text();
  ...
  let frag = `
      ...
      ${base_frag}
      float color(vec2 xy) {
          return snoise(xy + vec2(iTime, iTime) / 50.0);
      }
...
function NoiseRenderer({ className, id, color, opacity, density, complexity }: { className: string, id: string, color: number[], opacity: number, density: number, complexity: number}) {
  const noise_id = id;
  const color_array = color.join(',');
  const canvas_ref = React.useRef<HTMLCanvasElement>(null);
  React.useEffect(() => {
    if (canvas_ref.current) {
      const canvas = canvas_ref.current;
      render_noise(canvas, color, density, opacity, complexity);
    }
  });

  return (
    <canvas ref={canvas_ref} id={noise_id} className={`noise_canvas ` + className} color={color_array} opacity={opacity} density={density} complexity={complexity} rendered={"false"}></canvas>
  );
}

export { NoiseRenderer };