...
// Check if function "Diagram" is already defined
let EFFECTS = {
    '2d': {
        plain: `
 ...
 */
async function createFragShader(shape, sourceFile, glueFrag, effect, color, density, alpha, complexity) {
    let noiseFrag = await fetch(`${sourceFile}`);
    ...
    noiseFrag = noiseFrag.replace('const float complexity = 1.0;', `const float complexity = ${complexity};`, noiseFrag);
    ...
    return `${noiseFrag}
            // demo code:
            float color(${shape === '3d' ? 'vec3 xyz' : 'vec2 xy'}) {
                return ${glueFrag};
            }
 ...
 */
async function Diagram(selector, shape, sourceFile, glueFrag, effect, animate, color, density, alpha, complexity) {
    let shadertoyFrag = await createFragShader(shape, sourceFile, glueFrag, effect, color, density, alpha, complexity);
    let frag = `
